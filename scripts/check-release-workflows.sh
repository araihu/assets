...
require_contract(release_runs.include?('sha256sum --check --strict "$default_download/archive.sha256"') && release_runs.include?("unsafe default release archive member"), "release must hash-check and safely extract the promoted release")
